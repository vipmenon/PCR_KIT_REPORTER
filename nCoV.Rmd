...
## 主窗口表格内容
mainTbl_sql <- "SELECT specimenNumber `标本编号`, name `姓名`, gender `性别`,
                       specimenType `标本类型`, specimenStatus `标本状态`,
                       medicalNumber `病案号`, institution `送检单位`, originalNumber `原始编号`,
                       doctor `送检医师`, collectDate `采集日期`, receiveDate `接受日期`,
                       updatetime `更新日期`
                 FROM `Specimen`"
...
textInput("gender", label = "性别")

textInput("age", label = "年龄")

selectInput("specimenType", label = "标本类型", choices = c("", "鼻咽拭子", "手术组织", "穿刺组织", "全血",
                                    "血浆", "血清", "尿液", "粪便", "分泌物", "呕吐物",
                                    "唾液", "脑脊液", "淋巴液", "房水","其他"))
...
dateInput("collectDate", label = "采集日期", value = NA)

dateInput("receiveDate", label = "接受日期", value = Sys.Date())

actionButton("submit", label = "新增/更新")

query_id <- "specimenNumber"

update_ary <- c("name", "gender", "age", "specimenType", "specimenStatus", "medicalNumber", "institution", "originalNumber", "doctor", "collectDate", "receiveDate")
...
**姓 名：** `r renderText(input$name)` **性 别：** `r renderText(input$gender)` **年 龄：** `r renderText(input$age)`
**样本类型：** `r renderText(input$specimenType)` **样本状态：** `r renderText(input$specimenStatus)` **样本唯一标识：** `r renderText(input$specimenNumber)`

`r renderText(rv$permission[["insert"]])`
