...
CT_CUTOFF = list(HW = list(`L858R`=c(12,14), `19del`=c(8,11),
                                `T790M`=c(10,12), `L861Q`=c(12,15),
                                `G719X`=c(12,15), `Ins20`=c(11,14),
                                `S768I`=c(12,15), TAR_UP = 38,
                                CTRL_UP = 30),
                 YZY = list(`L858R`=c(7,7), `19DEL`=c(6,6),
...
pos_ctrl_text <- reactive(
  if (tbl_res_target()$CT[tbl_res_target()$`Sample Name` == input$POS_CTRL] %>% is.na() %>% any()) {
    ...
    if ((tbl_res_target()$CT[tbl_res_target()$`Sample Name` == input$POS_CTRL] <= CT_CUTOFF[[input$KIT_BRAND]][["CTRL_UP"]]) %>% all()) {
      "阳性对照品结果正常"
    } else {
      "阳性对照品结果异常"
    }
