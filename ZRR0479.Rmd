...
selectInput("tester", label = "检测员", choices = userNames, selected = 122953)
  ...
  withProgress(message = '开始分析数据...',
               detail = '依次分析所有样本结果...', value = 0, {
                for (sample in samples) {
                  ...
                  reportNumber <- paste0("P04", specimenNumber, "1")
                  
                  speInfo <- speNum2Info(specimenNumber)
                  
                  rv$reportStatus <- paste(rv$reportStatus, sample)
                  # rv$reportStatus <- paste(rv$reportStatus, sample, specimenNumber, ORF1ab, N, E, D, result,
                  #                          input$testDate, testerName, reporterName, reportNumber,
                  #                          speInfo$name)
                  
                  # 写入到Word报告中
                  dir.create(rv$dir)
                  
                  rpt <- read_docx("report/nCoV_template2.docx") %>%
