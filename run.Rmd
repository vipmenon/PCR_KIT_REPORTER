...
selectInput("testNumber", label = "检测项目", choices = get_select_list(mydb, "`Test`", "title", "testNumber"))
...
selectInput("userId", label = "实验员", choices = get_select_list(mydb, "`User`", "name", "userKey"))
...
observeEvent(input$clear, {
  if ( !is.na(as.integer(input$experimentNumber)) ) {
    updateTextInput(session, "experimentNumber", value = as.integer(input$experimentNumber) + 1)
  }
...
5. 【TAB】键跳转新增按钮，然后回车确认新增，新增记录会出翔在后侧第一条记录；

6. 此时实验号自动递增，准备进行下一个样本的登入；

7. 如果输入信息有误，需要更正或删除，在记录编号中输入记录号，然后更新内容，或点击删除。
