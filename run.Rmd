...
passwordInput("userId", label = "实验员号")
...
observeEvent(input$userKey, {
  ...
  if (rv$permission[["insert"]]) {
    ...
    updateTextInput(session, "userName", value = userName_lst[[input$userKey]])
  }
})
...
observeEvent({input$testNumber; input$date; input$experimentNumber}, {
  ...
  if (nrow(os) == 1) {
    ...
    updateTextInput(session, "specimenNumber", value = NA)
    updateTextInput(session, "userId", value = NA)
    updateTextInput(session, "userName", value = NA)
    updateTextInput(session, "specimenName", value = NA)
  }
})
