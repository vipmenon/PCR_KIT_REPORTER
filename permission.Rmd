...
observeEvent(input$userKey, {
  ...
  if (rv$permission[["view"]] & nrow(rv$table) > 0) {
    ...
    output$chartA <- renderPlotly(plot_ly(rv$table %>% group_by(operationId) %>% summarise(Num = length(operationId)),
                     x = ~`operationId`, y = ~`Num`, type = "bar"))
  } else {
...
observeEvent(input$submit, {
  ...
  submitValues <- paste(input$id, input$userId, input$tableId, input$operationId, input$isValid, "now()", sep = ", ")
...
observeEvent(input$delete, {
  delete_sql <- paste("DELETE FROM `", TABLE, "` WHERE `id` = ", input$id, sep = "")
