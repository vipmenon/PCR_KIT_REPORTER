...
# 中英模板
report_template_pos <- list(Liferiver_p = "report/nCoV_template6.docx",
                            Liferiver_h = "report/nCoV_template3.docx",
                            Biogerm_p = "report/nCoV_template7.docx")
report_template_neg <- list(Liferiver_p = "report/nCoV_template6.docx",
...
selectInput("tester", label = "检测员", choices = userNames, selected = 16359)

selectInput("checker", label = "审核员", choices = userNames, selected = 882287)
...
Result <- function(ORF1ab, N, E, D) {
  ...
  if (all(ORF1ab == "-", N == "+", E == "-") ) return("疑似近源冠状病毒")
  if (all(ORF1ab == "-", N == "-", E == "+") ) return("疑似近源冠状病毒")
...
ResultEN <- function(ORF1ab, N, E, D) {
  ...
  if (all(ORF1ab == "-", N == "+", E == "-") ) return("Suspected near source coronavirus")
  if (all(ORF1ab == "-", N == "-", E == "+") ) return("Suspected near source coronavirus")
  ...
  withProgress(message = '开始分析数据...',
               detail = '依次分析所有样本结果...', value = 0, {
                for (sample in samples) {
                  ...
                  #   pull(`CT`) %>% CT2Res(CT_CUTOFF = CT_CUTOFF[[input$KIT_BRAND]])
                  # E <- tbl_res() %>%
